...
const createNewAutomergeDocument = () => {
  resetNoteManager();
  const docUrl = automerger.createNoteDocument();
  select(".terminal").setValue("").focus();
  console.log("createNoteDocument docUrl 👉", docUrl);
  return url.set(docUrl);
};
...
export const commands = () => {
  return [
    ...
    },
    {
      ...
      call: async () => {
        commander.hide();
        const noteText = select(".terminal").getValue();
        automerger.saveNote({
          text: noteText,
        });
        setSavedState();
